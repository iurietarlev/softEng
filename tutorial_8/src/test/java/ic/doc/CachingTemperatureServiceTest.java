...
public class CachingTemperatureServiceTest {
  ...
  public void preventsMultipleRequestsDownstreamForSameParameters(){
    TemperatureService tempService = new CachingTemperatureService(downstream);

    context.checking(new Expectations(){{
      oneOf(downstream).temperatureFor("London", DayOfWeek.MONDAY); will(returnValue(11));
    }});
  ...
  public void evictsEldestMembersWhenCacheReachesCapacity(){
    ...
    context.checking(new Expectations(){{
      ...
      oneOf(downstream).temperatureFor("London", DayOfWeek.TUESDAY); will(returnValue(11));
    }});
